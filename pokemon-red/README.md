...
Everything for this game lives under `pokemon-red/`; the shared pieces (`fly-ai/`,
`data/`) sit in the repository root.
...
pokemon-red/
  ...
  render/
    ...
    record_fly.py         fly-driven rollout -> render/fly_drive.npz
...
Run these from the repository root (the venv and `requirements.txt` are shared by
every game):
...
models). Clone it into the repository root:
...
not included. Put it at `pokemon-red/roms/pokered.gb`. The RAM addresses in
`pokesim/adapter.py` match the [pret/pokered](https://github.com/pret/pokered)
disassembly for this dump:

```
SHA1  ea9bcae617fdf159b045185467ae58b2e4a48b9a
```

(The optional `pokemon-red/reference/pokered/pokered.sym`, from that repo, lets
you read state by symbol name; I read numeric RAM addresses instead.)

### 6.5 Scenes

`pokemon-red/roms/scenes/*.state` are PyBoy save states I bookmarked during
experiments and they are not committed. Any script can create one:

```sh
python pokemon-red/play_live.py --make-scene overworld   # walks the intro, bookmarks the bedroom
```
...
```sh
python pokemon-red/render/record_journey.py --window SDL2 --scale 4
```
...
```sh
python pokemon-red/render/record_journey.py   # writes render/recording.npz (~65 s)
python pokemon-red/render/render_journey.py   # writes media/viridian_final.mp4 (~2 min)
```

### Record the fly actually driving

```sh
python pokemon-red/render/record_fly.py --scene pallet --steps 80   # writes render/fly_drive.npz
python pokemon-red/render/render_journey.py \
    --input render/fly_drive.npz --output media/fly_drive.mp4 \
    --caption "the fly is driving: it only picks left/right"
```
...
sys.path.insert(0, "pokemon-red"); sys.path.insert(0, "fly-ai")
